 ...
 */
package com.github.shyiko.mysql.binlog.deserialization.json;

import com.github.shyiko.mysql.binlog.deserialization.AbstractRowsEventDataDeserializer;
import com.github.shyiko.mysql.binlog.model.type.ValueType;
...
public class JsonBinary {
     ...
     * @throws IOException if there is a problem reading or processing the binary representation
     */
    public static void parse(byte[] bytes, JsonFormatter formatter) throws IOException { new JsonBinary(bytes).parse(formatter); }

    private final ByteArrayInputStream reader;

    public JsonBinary(byte[] bytes) { this(new ByteArrayInputStream(bytes)); }
